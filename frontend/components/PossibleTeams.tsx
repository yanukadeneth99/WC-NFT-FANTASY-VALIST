...
export const getPrediction = async (
  ...
  let level, team, evolveStatus2, evolveStatus3, evolveStatus4

  const predictionOne = await PredictionContract.getPrediction(address, number);
  team = ethers.utils.defaultAbiCoder.decode(["string"], predictionOne)[0];
  evolveStatus2 = await EvolveContract.haveYouEvolvedAlready(team, 2);
  evolveStatus3 = await EvolveContract.haveYouEvolvedAlready(team, 3);
  evolveStatus4 = await EvolveContract.haveYouEvolvedAlready(team, 4);
  
  if(evolveStatus4 === true) {
    level = "4";
  } else if (evolveStatus3 === true) {
    level = "3";
  } else if (evolveStatus2 === true) {
     level = "2";
  } else {
    level = "1";
  }
  ...
  if (
    ...
    if (level === "3") return Argentina3;
    if (level === "2") return Argentina2;
    if (level === "1") return Argentina;
  }
